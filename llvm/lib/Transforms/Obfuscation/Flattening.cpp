...
static cl::opt<unsigned> FlattenMaxBB("cffobf-max-bb", cl::init(256),
    cl::NotHidden, cl::desc("Skip flattening for functions with more BBs"));
...
bool Flattening::runOnFunction(Function &F) {
  ...
  if (toObfuscate(flag, tmp, "fla") && !F.isPresplitCoroutine()) {
    // Skip functions with probe-stack (Rust/stack probing)
    if (F.hasFnAttribute("probe-stack"))
      return true;
    // Skip Swift async/continuation functions
    if (F.hasFnAttribute("swiftasync") || F.hasFnAttribute("swifttailcall"))
      return true;
    unsigned BBCount = 0;
    for ([[maybe_unused]] const BasicBlock &BB : F)
      BBCount++;
    if (BBCount > FlattenMaxBB) {
      errs() << "Skipping ControlFlowFlattening On " << F.getName()
             << " (too many BBs: " << BBCount << ")\n";
      return true;
    }
...
void Flattening::flatten(Function *f) {
  ...
  // Use 16-byte alignment on Darwin ARM64 for stack alignment
  Module *Mod = f->getParent();
  bool NeedAlign16 = isDarwinArm64(Mod);
  ...
  if (NeedAlign16) {
    switchVar->setAlignment(Align(16));
    switchVarAddr->setAlignment(Align(16));
  }
