...
struct BogusControlFlow : public FunctionPass {
  ...
  BasicBlock *createAlteredBasicBlock(BasicBlock *basicBlock,
    ...
    if (JunkAssemblyTemp || OnlyJunkAssemblyTemp) {
      ...
      // On Darwin ARM64, use NOP instead of random .long - random 32-bit values
      // can be interpreted as instructions that clobber x16/x17 (syscall) or
      // cause illegal/undefined behavior
      bool UseSafeAsm = isDarwinArm64(Mod);
      uint32_t junkCount = cryptoutils->get_range(MinNumberOfJunkAssemblyTemp,
                                                  MaxNumberOfJunkAssemblyTemp);
      for (uint32_t i = junkCount; i > 0; i--) {
        if (UseSafeAsm)
          junk += "nop\n";
        else
