...
static cl::opt<bool>
    EnableObjCRename("enable-objcrenamer", cl::init(false), cl::NotHidden,
                     cl::desc("Enable ObjC Symbol Renaming."));
...
static void LoadEnv(void) {
  ...
  if (getenv("OBJCRENAMER")) {
    EnableObjCRename = true;
  }
}
namespace llvm {
struct Obfuscation : public ModulePass {
  ...
  bool runOnModule(Module &M) override {
    ...
    MP->runOnModule(M);
    delete MP;
    // ObjC Rename (before string encryption so we can scan strings)
    MP = createObjCRenamePass(EnableAllObfuscation || EnableObjCRename);
    MP->runOnModule(M);
    delete MP;
    // Now Encrypt Strings
...
INITIALIZE_PASS_DEPENDENCY(ObjCRename);
